...
//                        curObject.setString("className", pkgName + "." + CaseFormat.UPPER_CAMEL.to(CaseFormat.UPPER_CAMEL, curObject.getString("name")));
	 ...
	 * @return
	 * @throws com.icg.utils.icgexception.ICGAppException
	 */
	...
	public static String convertToWsdl(String fullUrl, ApiObject apiService,
			  ApiClasses classes) throws Exception {
	...
	public static String convertToHtml(String fullUrl, ApiObject apiService,
		...
		for (ApiObject func : apiService.getList("functions")) {
			...
			sb.append(convertClassToHtml(inboundClassDef, classes, false));

			sb.append("<h3>Response</h3>");

			sb.append(convertClassToHtml(outboundClassDef, classes, false));

			sb.append("<h3>Exception</h3>");

			sb.append(convertClassToHtml(exceptionClassDef, classes, false));
		}
	...
	public static String convertClassToHtml(String curClass, ApiClasses classes, boolean position) {
		StringBuilder sb = new StringBuilder();
		...
		sb.append("<th>Field Type</th><th>Required</th><th>Description</th></tr></thead>");
		...
		for (ApiObject fld : thisClass.getList("fields")) {
			...
			if (position) {
				...
				sb.append("C:").append(iCnt).append(" ");
				sb.append("E:").append(getExcelColumnName(iCnt)).append(" ");

				if (fld.isSet("length")) {
					sb.append("F:").append(iFixedCnt).append(" ");
					sb.append("L:").append(fld.getLong("length")).append(" ");
					iFixedCnt += fld.getLong("length");
				}
				sb.append("</td>\n");
			}
			...
			if ("arraylist".equalsIgnoreCase(fld.getString("type")) || "object".
				...
				sb.append(convertClassToHtml(fld.getString("subType"), classes, position));
				sb.append("</td></tr>");
			}
